/**
 * CONSOLIDATED HR/PAYROLL/ATTENDANCE MODULE
 * Consolidates all HR-related endpoints from:
 ...
 * - /api/attendance (generic CRUD)
 * - /api/payroll (generic CRUD)
 * 
 * Single source of truth for all HR operations
 */

export const setupHRConsolidatedModule = (app, prisma, authenticateToken, authorize) => {
  console.log("🧑‍💼 Setting up Consolidated HR/Payroll/Attendance Module...");
  ...
  app.post("/api/hr/employees", authenticateToken, authorize(["admin", "super_admin"]), async (req, res) => {
    const { name, email, phone, department, position, salary, joinDate, status, companyId } = req.body;
  ...
  app.get("/api/hr/leaves", authenticateToken, async (req, res) => {
    try {
      ...
      const total = await prisma.leave.count({ where }).catch(() => 0);
  ...
  app.post("/api/hr/attendance", authenticateToken, async (req, res) => {
    const { employeeId, date, inTime, outTime, status, companyId } = req.body;
  ...
  app.get("/api/hr/attendance", authenticateToken, async (req, res) => {
    try {
      const { limit = 10, offset = 0, employeeId, companyId, status } = req.query;
      const where = {};
      ...
      const total = await prisma.attendance.count({ where }).catch(() => 0);
  ...
  app.post("/api/hr/payroll", authenticateToken, authorize(["admin", "super_admin"]), async (req, res) => {
    const { employeeId, month, basicSalary, bonus, deductions, status, companyId } = req.body;
  ...
  app.get("/api/hr/payroll", authenticateToken, async (req, res) => {
    try {
      ...
      const total = await prisma.payroll.count({ where }).catch(() => 0);
  ...
  console.log("✅ Consolidated HR/Payroll/Attendance Module setup complete!");
};
