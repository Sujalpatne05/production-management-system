...
const AddProductCategory = () => {
  ...
  const { addProductCategory } = useStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addProductCategory({ name: form.name, description: form.description });
    toast({ title: "Success", description: "Category added successfully" });
    navigate("/dashboard/item-setup/product-categories");
  };

  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold">Add Product Category</h1>
      <Card>
        <CardHeader>
