...
const SupplierLedger = () => {
  ...
  const ledgerData = useMemo(() => {
    ...
    suppliers.forEach((supplier) => {
      ledgers[supplier.id] = { name: supplier.name, entries: [] };
    });
    ...
    sortedPurchases.forEach((purchase) => {
      if (ledgers[purchase.supplierId]) {
        ledgers[purchase.supplierId].entries.push({
          date: purchase.date,
          invoiceNo: purchase.invoiceNo,
          amount: purchase.total,
          paid: purchase.paid,
          balance: purchase.due,
        });
      }
    });

    const result: SupplierLedger[] = Object.entries(ledgers)
      .map(([supplierId, data]) => ({
        supplierId,
    ...
    <div className="space-y-6">
      ...
      {ledgerData.length === 0 ? (
        ...
        ledgerData.map((ledger) => (
          <Card key={ledger.supplierId}>
            <CardHeader>
              <CardTitle>{ledger.supplierName}</CardTitle>
            </CardHeader>
            <CardContent>
